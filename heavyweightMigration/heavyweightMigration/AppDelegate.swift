...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func setEmployee(name : [String], id : [String], aadharNo : [String], doj : [String], address : [String]) {
        let context = self.persistentContainer.viewContext
        if let url = persistentContainer.persistentStoreCoordinator.persistentStores.first?.url {
            print("SQLite File URL: \(url)")
        }
        ...
        for i in 0..<Int(name.count) {
            ...
            managedOb.setValue(name[i], forKey: "name")
            managedOb.setValue(id[i], forKey: "id")
            managedOb.setValue(aadharNo[i], forKey: "panNo")
            managedOb.setValue(address[i], forKey: "address")
            managedOb.setValue(doj[i], forKey: "dateOfJoining")
        }
